    ...
    **kwargs: dict,
    ...
    if method == "mean":
        ...
        return _compute_consumer_optimal_solution_cvar_relaxed_naive_rounding(
            rel_matrix, k_rec, producer_max_min_utility, gamma, **kwargs
        )
...
def _compute_consumer_optimal_solution_cvar(
    ...
    prob.solve(solver=solver, warm_start=True, **{"MIPGap": 1e-3})
...
def _compute_consumer_optimal_solution_cvar_relaxed_topk_rounding(
    ...
    print("% of producer min util viol:", np.sum(alls.sum(axis=0) < producer_max_min_utility * gamma) / allocations.shape[1])
